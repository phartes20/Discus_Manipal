    ...
    root <- "C:/Users/partes/My Drive/People_2/Azwan Ismail/Discus_IV/Discus_Manipal/"
   ...
   # root <- "/users/visitor/artes/public_html/"
    ...
    filenames <- list.files(path = paste (root, "uploads/", sep=""), pattern=".xls", all.files = FALSE, full.names = FALSE, recursive = FALSE)
    
    goldstd <- read.spss (file=paste (root, "/key/goldstandard.sav", sep=""), to.data.frame=TRUE, trim_values=TRUE)
    
    
    names (goldstd) <- tolower (names(goldstd))
    
    goldstd$status <- as.factor ( goldstd$status)
    
    goldstd$status <- as.numeric(goldstd$status)-1
    names (goldstd)[3] <- "score"
    # names (goldstd) <- tolower (names(goldstd))
    goldstd$imgname <- trimws (as.character(goldstd$imgname))
    ...
    plotROCCurveForDamian <- function (fname, plotlabel) {
          ...
          d$status2 <- as.factor (d$status2)
          
          d$status2 <- as.integer(d$status2)-1
